...
def handle_response():
    ...
    if question_id == 0:
        ...
        first_name = words[1] if words[0].lower() in ["my", "i'm", "i"] and len(words) > 1 else words[0] if words else "there"
        conversation_log.append(f"Full name: {transcript}")
    else:
        conversation_log.append(f"{first_name}: {transcript}")

    # Use GPT to decide next question or wrap up
    try:
        system_prompt = (
    ...
    if "thank you, that's all i need today" in next_line.lower():
        ...
        <Response>
            <Say>{next_line}</Say>
            <Record maxLength="10" action="/handle-response?q={question_id + 1}" method="POST" playBeep="false" />
        </Response>
        """
